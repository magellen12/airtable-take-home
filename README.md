...
quarters** — and all three of those accounts score **1 out of 5 on Proof**: nobody can state,
in the buyer's language, what the customer actually got.

I scored the whole book on four dimensions. Capability averages **3.2/5**. Proof averages
**1.3** and Governance **1.5**. In no account is the blocker "Airtable can't do this."

**The platform is working. The operating model around it isn't.** That premise drives
...
| — | Methodology | [`docs/03-transformation-methodology.md`](docs/03-transformation-methodology.md) | The repeatable motion: maturity ladder, constraint rule, and the 8-play library. |
| — | Operating model | [`docs/04-operating-model.md`](docs/04-operating-model.md) | How this runs across 15 CSMs — ownership boundaries, rhythm, metrics, inspection, and how I'd develop the team. |
...
| — | Seed data | [`airtable-build/data/`](airtable-build/data/) | Import-ready CSVs — the six accounts pre-scored, 8 plays, stakeholders, signals, value stories. |
...
commercial signals feed an AI-drafted **value story** in the buyer's own language, which is
what the CSM carries into renewal. Two interfaces: a **CSM Cockpit** (my book, my constraint,
my next move) and a **Director Book Review** (the inspection surface a team that just lost its
manager layer doesn't have).
